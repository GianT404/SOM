...
func (a *App) renderTransferPopup() string {
	...
	b.WriteString("\n")
	if info.Paired {
		b.WriteString(StatusOKStyle.Render("  Mobile device paired • sync server is running"))
	} else {
		b.WriteString(StatusOKStyle.Render("  Direct device sync is waiting for mobile"))
	}
	b.WriteString("\n\n")
